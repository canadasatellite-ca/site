...
class ImportTemplateHandler
{
    ...
    protected function validateOptionAttributes($data)
    {
        ...
        foreach ($optionAttributes as $optionAttribute) {
            ...
            $optionAttribute->validateTemplateImportMageTwo($data);
        }
    }
    ...
    protected function validateValueAttributes($data)
    {
        ...
        foreach ($valueAttributes as $valueAttribute) {
            ...
            $valueAttribute->validateTemplateImportMageTwo($data);
        }
    }
    ...
    protected function collectSystemDataFromOptions($data)
    {
        ...
        foreach ($options as $optionData) {
            $this->collectStoresDataByKey($optionData, 'mageworx_title');
            $this->collectStoresDataByKey($optionData, 'description');
            $this->collectStoresDataByKey($optionData, 'mageworx_option_price');
            $this->collectStoresDataByKey($optionData, 'store_view');
            $this->collectCustomerGroupsDataByKey($optionData, 'customer_group');
            ...
            foreach ($optionData['values'] as $valueData) {
                $this->collectStoresDataByKey($valueData, 'mageworx_title');
                $this->collectStoresDataByKey($valueData, 'description');
                $this->collectStoresDataByKey($valueData, 'mageworx_option_type_price');
            }
        }
    }
    ...
    protected function collectStoresDataByKey($data, $key)
    {
        ...
        foreach ($decodedData as $decodedDatum) {
            if (!empty($decodedDatum['store_id'])) {
                $this->storeIdMap[$decodedDatum['store_id']] = $decodedDatum['store_id'];
            } elseif (!empty($decodedDatum['customer_store_id'])) {
                $this->storeIdMap[$decodedDatum['customer_store_id']] = $decodedDatum['customer_store_id'];
            }
        }
    }
    ...
    protected function collectCustomerGroupsDataByKey($data, $key)
    {
        ...
        foreach ($decodedData as $decodedDatum) {
            ...
            $this->customerGroupMap[$decodedDatum['customer_group_id']] = $decodedDatum['customer_group_id'];
        }
    }
    ...
    {
        if (!isset($fromOption['option_id']) || !isset($fromOption['description'])) {
            return;
        }
        ...
        foreach ($fromOption['values'] as &$value) {
            ...
            if (!isset($value['option_type_id']) || !isset($value['description'])) {
                continue;
            }
    ...
    {
        if (!isset($fromOption['option_id']) || !isset($fromOption['mageworx_title'])) {
            return;
        }
        ...
        foreach ($fromOption['values'] as &$value) {
            ...
            if (!isset($value['option_type_id']) || !isset($value['mageworx_title'])) {
                continue;
            }
    ...
    {
        if (!isset($fromOption['option_id']) || !isset($fromOption['mageworx_option_price'])) {
            return;
        }
        ...
        foreach ($fromOption['values'] as &$value) {
            ...
            if (!isset($value['option_type_id']) || !isset($value['mageworx_option_type_price'])) {
                continue;
            }
    ...
    {
        if (!isset($optionData['store_view']) || !is_string($optionData['store_view'])) {
    ...
    {
        if (!isset($optionData['customer_group']) || !is_string($optionData['customer_group'])) {
