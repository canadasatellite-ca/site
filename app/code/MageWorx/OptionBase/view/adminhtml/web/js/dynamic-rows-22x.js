...
define([
    ...
    return uiCollection.extend({
        defaults: {
            ...
            recordDataCache: [],
            startIndex: 0,
            notRequiredFields: ['price', 'qty', 'cost', 'weight', 'description']
        },
        ...
        restoreRecordId: function () {
            ...
            if (!_.isUndefined(options) && options.length > 0) {
                jQuery(options).each(function (optionKey, optionData) {
                    if (!_.isUndefined(optionData.values) && optionData.values.length > 0) {
                        jQuery(optionData.values).each(function (valueKey, valueData) {
        ...
        setClasses: function (data) {
            ...
            // fix Magento bug: do not set required fields from the notRequiredFields array
            if (_.contains(this.notRequiredFields, data.name)) {
                data.required = false;
            }
