 ...
 */
namespace MageSuper\CustomizableOptionsPatch\Model\ResourceModel\Product\Option;
/**
...
class Value extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    protected function _saveValuePrices(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if ($object->getPrice() && $priceType) {
            ...
            $optionTypeId = $this->getConnection()->fetchOne($select) ? $this->getConnection()->fetchOne($select) : (string)$object->getId();
            if ($optionTypeId) {
    ...
    protected function _saveValueTitles(\Magento\Framework\Model\AbstractModel $object)
    {
        foreach ([\Magento\Store\Model\Store::DEFAULT_STORE_ID, $object->getStoreId()] as $storeId) {
            ...
            $optionTypeId = $this->getConnection()->fetchOne($select) ? $this->getConnection()->fetchOne($select) : (string)$object->getId();
            $existInCurrentStore = $this->getOptionIdFromOptionTable($titleTable, (int)$object->getId(), (int)$storeId) ? $this->getOptionIdFromOptionTable($titleTable, (int)$object->getId(), (int)$storeId) : $optionTypeId;
            if ($object->getTitle()) {
