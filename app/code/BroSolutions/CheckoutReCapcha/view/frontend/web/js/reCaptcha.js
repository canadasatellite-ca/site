...
define(
    ...
    function (Component, urlBuilder, $, ko, registry, undefined) {
        ...
        return Component.extend({
            ...
            reCaptchaCallback: function (token) {
                window.checkout.kjip = null;
                window.checkout.adhs = null;
                if (this.settings.size === 'invisible') {
                    this.tokenField.value = token;
                    setTimeout(function () {
                        if (token !== '') {
                            $.ajax({
                                url: urlBuilder.build('checkoutrechaptcha/verify/index'),
                                data: {'token': token},
                                type: 'post',
                                success: function (response) {
                                    if (response.status == true) {
                                        window.checkout.kjip = true;
                                        window.checkout.adhs = token;
                                    } else {
                                        window.checkout.kjip = false;
                                    }
                                },
                                fail: function () {
                                    window.checkout.kjip = false;
            ...
            initCaptcha: function () {
                ...
                if (this.settings.size === 'invisible' && $parentForm.length > 0) {
                    ...
                    this.tokenField = $('<input type="text" name="token" style="display: none" value="" />')[0];
