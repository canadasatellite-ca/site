...
namespace CanadaSatellite\Theme\Model\Action;
...
class Export
{
    ...
    function process($request, $storeId)
    {
        try {
            ...
            $websitesStores = array();
            $storeIds = array();

            foreach ($storeManagerDataList as $store) {
                $websitesStores[$store->getWebsiteId()][] = $store->getId();
            }

            foreach ($websitesStores as $key => $value) {
                if ($key === (int)$websiteId) {
                    $storeIds = $value;
                    break;
                }
            }
            ...
            if ($startDate && $endDate) {
                ...
                if (!empty($storeIds)) {
                    $orders->addAttributeToFilter('store_id', ['in' => $storeIds]);
                } else {
