...
class ValidateAddToCart
{
    ...
    public function beforeValidateUserValue(DefaultType $subject, $values)
    {
        ...
        $processingParams = $subject->getRequest()->getData('_processing_params');

        if (isset($processingParams) && $option->getType()=='date' && !isset($_SESSION['admin']['user'])){
            $values = $processingParams->getCurrentConfig()->getOptions();
        }
        ...
        foreach ($this->validationResolver->getValidators() as $validatorItem) {
            if (!$validatorItem->canValidateAddToCart($subject, $values) && $option->getType()!=='date') {
