...
class AroundGetOptionPrice
{
        ...
        if (!$this->isSingleSelection($option)) {
            foreach (explode(',', $optionValue) as $_value) {
                ...
                if ($_result) {
                    ...
                    $result .= $this->setTitle($_result, $titleQty, 0, $currencyCode);
                } else {
            ...
            if ($_result) {
                ...
                $titleQty = $this->getTitleQty($subject, $optionQty);
                $result .= $this->setTitle($_result, $titleQty, 1, $currencyCode);
            } else {
                if ($subject->getListener()) {
                    $subject->getListener()->setHasError(true)->setMessage($this->getWrongConfigurationMessage());
                }
    ...
    public function aroundGetOptionPrice($subject, $proceed, $optionValue, $basePrice)
    {
        ...
        $currencyCode = $option->getCurrencyCode();
        $optionCurrencyRate = $this->currencyModel->getCurrencyRates($currencyCode, 'CAD')['CAD'];
        $frontendCurrency = $this->storeManager->getStore()->getCurrentCurrency()->getCode();

        if ($frontendCurrency !== $currencyCode) {
            $result = $result * $optionCurrencyRate * CasatCurrency::EXCHANGE_RATE;
        } else {
            $result = $result * $optionCurrencyRate;
        }
     ...
     * @return string
     */
    protected function setTitle($model, $qty, $isSingleSelection, $currencyCode)
    {
        $title = '';
        if ($qty > 1) {
            $title .= $qty .' x ';
        ...
        $title .= $this->getOptionPriceAsString($model, $qty, $currencyCode);
     ...
     * @return string
     */
    protected function getOptionPriceAsString($model, $qty, $currencyCode)
    {
        $actualPrice = null;
        ...
        $frontendCurrency = $this->storeManager->getStore()->getCurrentCurrency()->getCode();
        if ($frontendCurrency !== $currencyCode) {
            $price = $price * $optionCurrencyRate * CasatCurrency::EXCHANGE_RATE;
        } else {
            $price = $price * $optionCurrencyRate;
        }
