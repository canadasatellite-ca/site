        ...
        if (intval($currentStoreId) != 0 && $product->getId()) {
            ...
            foreach ($productData as $productKey => $productValue) {
                ...
                if ($forceDefault) {
                    ...
                    /*
                     * Fix for global value
                     */

                    if ($attribute->getIsGlobal()) {
                        $defaultScopeValue = $rootProduct->getData($attribute->getAttributeCode());
                        if ($defaultScopeValue) {
                            $product->setData($productKey, $defaultScopeValue);
                        } else {
                            $product->setData($productKey, $attribute->getData('default_value'));
                        }
                    } else {
                        /*
