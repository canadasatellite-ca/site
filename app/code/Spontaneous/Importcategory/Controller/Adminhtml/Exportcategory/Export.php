...
class Export extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        $content .= ',"store"';
        $content .= ',"name","path","image","is_active","is_anchor","include_in_menu","meta_title","meta_keywords","meta_description","display_mode","custom_use_parent_settings","custom_apply_to_products","custom_design","custom_design_from","custom_design_to","default_sort_by","page_layout","description","volusion_url","alternate_url","products"'."\n";
        ...
        foreach ($collection as $key => $cat) {
            ...
            if ($cat->getId()>=2) {
                ...
                if ($categoryitem->getId()) {
                    ...
                    $content .= $categoryitem->getName().'","'.$categoryitem->getPath().'","'.$categoryitem->getImage().'","'.$categoryitem->getIsActive().'","'.$categoryitem->getIsAnchor().'","'.$categoryitem->getIncludeInMenu().'","'.$categoryitem->getMetaTitle().'","'.$categoryitem->getMetaKeywords().'","'.$categoryitem->getMetaDescription().'","'.$categoryitem->getDisplayMode().'","'.$categoryitem->getCustomUseParentSettings().'","'.$categoryitem->getCustomApplyToProducts().'","'.$categoryitem->getCustomDesign().'","'.$categoryitem->getCustomDesignFrom().'","'.$categoryitem->getCustomDesignTo().'","'.$categoryitem->getDefaultSortBy().'","'.$categoryitem->getPageLayout().'","'.$categoryitem->getDescription().'","'.$categoryitem->getVolusionUrl().'","'.$categoryitem->getAlternateUrl().'","'.$prodids.'"'."\n";
                }
            }
        }
