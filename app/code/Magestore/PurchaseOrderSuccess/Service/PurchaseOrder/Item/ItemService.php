...
class ItemService
{
     ...
     * @return bool
     */
    public function addProductToPurchaseOrder($purchaseId, $productsData = [],$rate = null){
        ...
        if(!$rate){
            $rate = $purchaseOrder->getCurrencyRate();
        }
        $purchaseProductsData = $this->prepareProductDataToPurchaseOrder(
            $purchaseId, $productsData, $purchaseProductIds, [],$rate
        );
