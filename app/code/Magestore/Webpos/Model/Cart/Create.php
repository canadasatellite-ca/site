             ...
             implements \Magestore\Webpos\Api\Cart\CheckoutInterface
{
     ...
     * @return $this
     */
    public function initQuote($quoteId)
    {
        if($quoteId){
...
//        if(!$quoteId) {
//            $this->initQuote($this->getQuote()->getId());
//            $this->saveQuote();
//        }
        $session = $this->_objectManager->create('\Magento\Checkout\Model\Session');
    ...
    public function getCartTotals(){
        ...
        if($this->_objectManager->get('\Magento\Framework\Module\Manager')
            ...
            foreach ($totals as $total) {
                $data = $total->getData();
                if($total->getData('code') == 'rewardpoints_label') {
    ...
    public function getShipping(){
        ...
        $this->allowShippings = array('webpos_shipping','flatrate','freeshipping');
        if(count($list) > 0){
            ...
            foreach ($list as $data) {
                ...
                if (!in_array($carrierCode, $this->allowShippings))
                    continue;
                if (!$shippingHelper->isAllowOnWebPOS($carrierCode))
                    continue;
                $isDefault = '0';
...
//                $methodSpecificerrmsg = ($data->getErrorMessage() != null) ? $data->getErrorMessage() : '';
    ...
    public function getPayment(){
        ...
        $this->allowPayments = array('cashforpos', 'codforpos', 'ccforpos', 'cp1forpos', 'cp2forpos',
                            'paypal_direct', 'authorizenet_directpost', 'cryozonic_stripe', 'payflowpro');
        $this->ccPayments = array('authorizenet_directpost', 'cryozonic_stripe', 'payflowpro');
        $paymentList = array();
        if(count($list) > 0) {
        ...
        $isPaypalEnable = $paymentHelper->isPaypalEnable();
        if($isPaypalEnable) {
    ...
    public function placeOrder($quoteId, $payment, $shippingMethod, $quoteData, $actions, $integration, $extensionData)
    {
        ...
        try {
            ...
            if($paymentModel && !$this->isProcessPaymentBeforeOrder($payment['method'])) {
    ...
    public function applyGiftcard($quoteId, $code, $amount)
    {
        ...
        foreach ($addCodes as $code) {
            ...
            if (!$giftVoucher->getId() || $giftVoucher->getSetId()) {
                ...
                $error .= $errorMessage . ' ' . $maxErrorMessage;
        ...
        if ($codes = $session->getGiftCodes()) {
            ...
            foreach ($codesArray as $key => $value) {
                    ...
                    $giftVoucher = $this->_objectManager->create('Magestore\Giftvoucher\Model\Giftvoucher')
                                    ->loadByCode($value);
                    $discount['balance'] = abs(round($giftVoucher->getBalance(), 2));
                    $discounts[] = $discount;
    ...
    public function getPaymentRequest($quoteId, $payment){
        ...
        if($paymentModel){
            ...
            if($paymentMethod && $this->isProcessPaymentBeforeOrder($paymentMethod)){
                $result = array();
                $result['payment_infomation'] = [];
                $result['payment_model'] = $paymentMethod;
                $result['quote_id'] = $quoteId;
                $paymentModel = $this->getPaymentModelByCode($this->getPaymentModelCode($paymentMethod));
                if($paymentModel) {
