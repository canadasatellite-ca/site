...
//            }
    ...
    protected function _getOrdersExtQuery($orders) {
        ...
        $ext_query = array(
            ...
            'sales_flat_shipment' => "SELECT * FROM _DBPRF_sales_flat_shipment WHERE order_id IN {$order_id_query}",
            'sales_flat_creditmemo' => "SELECT * FROM _DBPRF_sales_flat_creditmemo WHERE order_id IN {$order_id_query}",
        );
        return $ext_query;
    }
    ...
    protected function _getOrdersExtRelQuery($orders, $ordersExt) {
        ...
        $shipmentIds = $this->duplicateFieldValueFromList($ordersExt['object']['sales_flat_shipment'], 'entity_id');
        $shipment_ids_query = $this->arrayToInCondition($shipmentIds);
        $creditmemoIds = $this->duplicateFieldValueFromList($ordersExt['object']['sales_flat_creditmemo'], 'entity_id');
        $creditmemo_ids_query = $this->arrayToInCondition($creditmemoIds);
        $ext_rel_query = array(
            ...
            ///
            'sales_flat_shipment_grid' => "SELECT * FROM _DBPRF_sales_flat_shipment_grid WHERE entity_id IN {$shipment_ids_query}",
            'sales_flat_shipment_comment' => "SELECT * FROM _DBPRF_sales_flat_shipment_comment WHERE parent_id IN {$shipment_ids_query}",
            'sales_flat_shipment_item' => "SELECT * FROM _DBPRF_sales_flat_shipment_item WHERE parent_id IN {$shipment_ids_query}",
            ///
            'sales_flat_creditmemo_grid' => "SELECT * FROM _DBPRF_sales_flat_creditmemo_grid WHERE entity_id IN {$creditmemo_ids_query}",
            'sales_flat_creditmemo_comment' => "SELECT * FROM _DBPRF_sales_flat_creditmemo_comment WHERE parent_id IN {$creditmemo_ids_query}",
            'sales_flat_creditmemo_item' => "SELECT * FROM _DBPRF_sales_flat_creditmemo_item WHERE parent_id IN {$creditmemo_ids_query}",
        );
        return $ext_rel_query;
    }
    ...
    public function afterSaveOrder($order_mage_id, $data, $order, $ordersExt) {
        ...
        if ($orderInvoice) {
            ...
            $invoice_data = array(
                ...
                'base_total_refunded' => isset($orderInvoice['base_total_refunded']) ? $orderInvoice['base_total_refunded'] : '',
                'discount_description' => isset($orderInvoice['discount_description']) ? $orderInvoice['discount_description'] : '',
        ...
        $bill_id = $orderMage->getBillingAddressId();
        $ship_id = $orderMage->getShippingAddressId();
        $orderShipment = $this->getRowFromListByField($ordersExt['object']['sales_flat_shipment'], 'order_id', $order['entity_id']);
        $orderDesc = $this->selectTableRow('sales_order', array('entity_id' => $order_mage_id));
        if($orderShipment){
//            $orderMage = $this->_objectManager->create('Magento\Sales\Model\Order')->load($order_mage_id);
//            $bill_id = $orderMage->getBillingAddressId();
//            $ship_id = $orderMage->getShippingAddressId();
            ...
            $shipment_data = array(
                'store_id' => $store_id,
                'total_weight' => $orderShipment['total_weight'],
                'total_qty' => $orderShipment['total_qty'],
                'email_sent' => $orderShipment['email_sent'],
                'send_email' => 1,
                'order_id' => $order_mage_id,
                'customer_id' => $this->getMageIdCustomer($orderShipment['customer_id']),
                'shipping_address_id' => $ship_id ? $ship_id : null,
                'billing_address_id' => $bill_id ? $bill_id : null,
                'shipment_status' => $orderShipment['shipment_status'],
                'increment_id' => $orderShipment['increment_id'],
                'created_at' => $orderShipment['created_at'],
                'updated_at' => $orderShipment['updated_at'],
                'packages' => NULL,
                'shipping_label' => "",
                'customer_note' => "",
                'customer_note_notify' => 1
            );
            $insert = $this->insertTable('sales_shipment', $shipment_data);
            if($insert){
                $entity_id = $this->selectTableMax('sales_shipment', 'entity_id');
                $shipment_grid = $this->getRowFromListByField($ordersExt['object']['sales_flat_shipment_grid'], 'entity_id', $orderShipment['entity_id']);
                $shipment_grid_data = array(
                    'entity_id' => $entity_id,
                    'increment_id' => $shipment_grid['increment_id'],
                    'store_id' => $store_id,
                    'order_increment_id' => $orderDesc['increment_id'],
                    'order_id' => $order_mage_id,
                    'order_created_at' => $shipment_grid['order_created_at'],
                    'customer_name' => $shipment_grid['shipping_name'],
                    'total_qty' => $shipment_grid['total_qty'],
                    'shipment_status' => $shipment_grid['shipment_status'],
                );
                $this->insertTable('sales_shipment_grid', $shipment_grid_data);
                $shipment_comment = $this->getListFromListByField($ordersExt['object']['sales_flat_shipment_comment'], 'parent_id', $orderShipment['entity_id']);
                foreach ($shipment_comment as $comments){
                    $shipment_comment_data = array(
                        'parent_id' => $entity_id,
                        'is_customer_notified' => $comments['is_customer_notified'],
                        'is_visible_on_front' => 0,
                        'comment' => $comments['comment'],
                        'created_at' => $comments['created_at']
                    );
                    $this->insertTable('sales_shipment_comment', $shipment_comment_data);
                }
                $shipment_items = $this->getListFromListByField($ordersExt['object']['sales_flat_shipment_item'], 'parent_id', $orderShipment['entity_id']);
                foreach ($shipment_items as $item){
                    $order_item = $this->selectTableRow('sales_order_item', array(
                        'order_id' => $order_mage_id,
                        'product_id' => $this->getMageIdProduct($item['product_id'])
                    ));
                    if(!$order_item){
                        $order_item = $this->selectTableRow('sales_order_item', array(
                            'order_id' => $order_mage_id,
                    ...
                    $shipment_item_data = array(
                        ...
                        'qty' => $item['qty'],
                        'product_id' => $this->getMageIdProduct($item['product_id']),
                        'order_item_id' => $order_item ? $order_item['item_id'] : '',
                        'additional_data' => $item['additional_data'],
                        'description' => $item['description'],
                        'name' => $item['name'],
                        'sku' => $item['sku']
                    );
                    $this->insertTable('sales_shipment_item', $shipment_item_data);
                }
            }
        }
        
        //Creditmemo
        $orderCreditmemo = $this->getRowFromListByField($ordersExt['object']['sales_flat_creditmemo'], 'order_id', $order['entity_id']);
        if($orderCreditmemo){
            $store_id = ($orderCreditmemo['store_id']) ? $this->_notice['config']['languages'][$orderCreditmemo['store_id']] : $this->_notice['config']['languages'][$this->_notice['config']['default_lang']];
            $creditmemo_data = array(
                'store_id' => $store_id,
                'adjustment_positive' => $orderCreditmemo['adjustment_positive'],
                'base_shipping_tax_amount' => $orderCreditmemo['base_shipping_tax_amount'],
                'store_to_order_rate' => $orderCreditmemo['store_to_order_rate'],
                'base_discount_amount' => $orderCreditmemo['base_discount_amount'],
                'base_to_order_rate' => $orderCreditmemo['base_to_order_rate'],
                'grand_total' => $orderCreditmemo['grand_total'],
                'base_adjustment_negative' => $orderCreditmemo['base_adjustment_negative'],
                'base_subtotal_incl_tax' => $orderCreditmemo['base_subtotal_incl_tax'],
                'shipping_amount' => $orderCreditmemo['shipping_amount'],
                'subtotal_incl_tax' => $orderCreditmemo['subtotal_incl_tax'],
                'adjustment_negative' => $orderCreditmemo['adjustment_negative'],
                'base_shipping_amount' => $orderCreditmemo['base_shipping_amount'],
                'store_to_base_rate' => $orderCreditmemo['store_to_base_rate'],
                'base_to_global_rate' => $orderCreditmemo['base_to_global_rate'],
                'base_adjustment' => $orderCreditmemo['base_adjustment'],
                'base_subtotal' => $orderCreditmemo['base_subtotal'],
                'discount_amount' => $orderCreditmemo['discount_amount'],
                'subtotal' => $orderCreditmemo['subtotal'],
                'adjustment' => $orderCreditmemo['adjustment'],
                'base_grand_total' => $orderCreditmemo['base_grand_total'],
                'base_adjustment_positive' => $orderCreditmemo['base_adjustment_positive'],
                'base_tax_amount' => $orderCreditmemo['base_tax_amount'],
                'shipping_tax_amount' => $orderCreditmemo['shipping_tax_amount'],
                'tax_amount' => $orderCreditmemo['tax_amount'],
                'order_id' => $order_mage_id,
                'email_sent' => $orderCreditmemo['email_sent'],
                'send_email' => 1,
                'creditmemo_status' => $orderCreditmemo['creditmemo_status'],
                'state' => $orderCreditmemo['state'],
                'shipping_address_id' => $ship_id ? $ship_id : '',
                'billing_address_id' => $bill_id ? $bill_id : '',
                'invoice_id' => NULL,
                'store_currency_code' => $orderCreditmemo['store_currency_code'],
                'order_currency_code' => $orderCreditmemo['order_currency_code'],
                'base_currency_code' => $orderCreditmemo['base_currency_code'],
                'global_currency_code' => $orderCreditmemo['global_currency_code'],
                'transaction_id' => $orderCreditmemo['transaction_id'],
                'increment_id' => $orderCreditmemo['increment_id'],
                'created_at' => $orderCreditmemo['created_at'],
                'updated_at' => $orderCreditmemo['updated_at'],
                'discount_tax_compensation_amount' => '',
                'base_discount_tax_compensation_amount' => '',
                'shipping_discount_tax_compensation_amount' => '',
                'base_shipping_discount_tax_compensation_amnt' => '',
                'shipping_incl_tax' => $orderCreditmemo['shipping_incl_tax'],
                'base_shipping_incl_tax' => $orderCreditmemo['base_shipping_incl_tax'],
                'discount_description' => '',
                'customer_note' => '',
                'customer_note_notify' => ''
            );
            $insert_creditmemo = $this->insertTable('sales_creditmemo', $creditmemo_data);
            if($insert_creditmemo){
                $entity_id = $this->selectTableMax('sales_creditmemo', 'entity_id');
                $creditmemo_grids = $this->getRowFromListByField($ordersExt['object']['sales_flat_creditmemo_grid'], 'entity_id', $orderCreditmemo['entity_id']);
                $store_id = ($creditmemo_grids['store_id']) ? $this->_notice['config']['languages'][$creditmemo_grids['store_id']] : $this->_notice['config']['languages'][$this->_notice['config']['default_lang']];
                $creditmemo_grid_data = array(
                    'entity_id' => $entity_id,
                    'increment_id' => $creditmemo_grids['increment_id'],
                    'created_at' => $creditmemo_grids['created_at'],
                    'updated_at' => NULL,
                    'order_id' => $order_mage_id,
                    'order_increment_id' => $orderDesc['increment_id'],
                    'order_created_at' => $creditmemo_grids['order_created_at'],
                    'billing_name' => $creditmemo_grids['billing_name'],
                    'state' => $creditmemo_grids['state'],
                    'base_grand_total' => $creditmemo_grids['base_grand_total'],
                    'order_status' => NULL,
                    'store_id' => $store_id,
                    'billing_address' => $bill_id ? $bill_id : '',
                    'shipping_address' => $ship_id ? $ship_id : '',
                    'customer_name' => $creditmemo_grids['billing_name'],
                    'customer_email' => NULL,
                    ...
                    'order_base_grand_total' => NULL
                );
                $this->insertTable('sales_creditmemo_grid', $creditmemo_grid_data);
                $creditmemo_items = $this->getListFromListByField($ordersExt['object']['sales_flat_creditmemo_item'], 'parent_id', $orderCreditmemo['entity_id']);
                foreach ($creditmemo_items as $item){
                    ...
                    $this->insertTable('sales_creditmemo_item', $creditmemo_item_data);
                }
                $creditmemo_comments = $this->getListFromListByField($ordersExt['object']['sales_flat_creditmemo_comment'], 'parent_id', $orderCreditmemo['entity_id']);
                foreach ($creditmemo_comments as $row){
                    $creditmemo_comment_data = array(
                        'parent_id' => $entity_id,
                        'is_customer_notified' => $row['is_customer_notified'],
                    ...
                    $this->insertTable('sales_creditmemo_comment', $creditmemo_comment_data);
                }
            }
        }
    }
