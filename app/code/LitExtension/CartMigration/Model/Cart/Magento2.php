...
class Magento2 extends \LitExtension\CartMigration\Model\Cart
{
    ...
    public function displayConfig() {		
        ...
        $default_cfg = $this->_getDataImport($this->_getUrlConnector('query'), array(
            'serialize' => true,
            'query' => serialize(array(
                "store" => "SELECT * FROM _DBPRF_store WHERE store_id != 0",
        ...
        if ($object && $object['store'] && $object['eav_entity_type']) {
            $this->_notice['config']['default_lang'] = $this->_getDefaultLanguage($object['store']);
        ...
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            "serialize" => true,
            "query" => serialize(array(
                "store" => "SELECT * FROM _DBPRF_store WHERE code != 'admin'",
        ...
        foreach ($obj['store'] as $language_row) {
    ...
    public function displayImport() {
        ...
        $data = $this->_getDataImport($this->_getUrlConnector('query'), array(
            ...
            'query' => serialize(array(
                ...
                'categories' => "SELECT COUNT(1) FROM _DBPRF_catalog_category_entity WHERE entity_id > {$this->_notice['categories']['id_src']} AND level > 1",
                'products' => "SELECT COUNT(1) FROM _DBPRF_catalog_product_entity WHERE entity_id > {$this->_notice['products']['id_src']}",
                'customers' => "SELECT COUNT(1) FROM _DBPRF_customer_entity WHERE entity_id >= 10562 AND entity_id <= 11681 AND entity_id > {$this->_notice['customers']['id_src']}",
                'orders' => "SELECT COUNT(1) FROM _DBPRF_sales_order WHERE entity_id >= 16314 AND entity_id <= 19311 AND entity_id > {$this->_notice['orders']['id_src']}",
                'reviews' => "SELECT COUNT(1) FROM _DBPRF_review WHERE review_id > {$this->_notice['reviews']['id_src']}",
            ))
        ));
        if (!$data || $data['result'] != 'success') {
    ...
    public function getProductsExt($products) {
        ...
        $ext_query = array(
            ...
            'catalog_product_entity_media_gallery' => "SELECT m.*, a.*, b.* FROM _DBPRF_catalog_product_entity_media_gallery_value_to_entity m LEFT JOIN _DBPRF_catalog_product_entity_media_gallery a ON m.value_id = a.value_id LEFT JOIN _DBPRF_catalog_product_entity_media_gallery_value b ON m.value_id = b.value_id WHERE b.store_id = 0 AND m.entity_id IN {$pro_id_query}",
        ...
        $ext_rel_query = array(
            ...
            'core_store' => "SELECT * FROM _DBPRF_store WHERE code != 'admin'",
    ...
    public function convertProduct($product, $productsExt) {
        ...
        if ($product['type_id'] == 'configurable') {
            ...
            foreach ($attributeAll as $row) {
                ...
                $attr_import = $this->_makeAttributeImport($attribute, array(), $product['attribute_set_id'], $info, $entity_type_id, true);
...
//                    );
            ...
            foreach ($parent_products as $parent) {
                ...
                if ($id_parent) {
                    ...
                    if ($type_id == 'configurable') {
                        ...
                        $add_data = $optionData = $selectionData = array();
                        $model_type = $model_parent->getTypeInstance();
                        //$used_product_ids = $model_type->getChildrenIds($id_parent);
                        $allOptions = $model_type->getOptions($model_parent);
                        foreach ($allOptions as $row) {
                            $option = $row->getData();
                            $option['delete'] = '';
                            $optionData[] = $option;
                            $group_select = array();
                            $selectionsOption = $this->selectTable('catalog_product_bundle_selection', array('parent_product_id' => $id_parent, 'option_id' => $option['option_id']));
                            foreach ($selectionsOption as $value) {
                                $value['delete'] = '';
                                $group_select[] = $value;
                            }
                            $srcSelections = $this->getListFromListByField($productsExt['object']['catalog_product_bundle_selection'], 'parent_product_id', $parent['parent_id']);
                            $srcSelection = $this->getRowFromListByField($srcSelections, 'product_id', $product['entity_id']);
                            $title = $this->getRowValueFromListByField($productsExt['object']['catalog_product_bundle_option_value'], 'option_id', $srcSelection['option_id'], 'title');
                        ...
                        $add_data['bundle_options_data'] = $optionData;
                        $add_data['bundle_selections_data'] = $selectionData;
                        $add_data['can_save_custom_options'] = true;
                        $add_data['can_save_bundle_selections'] = true;
                        $add_data['affect_bundle_product_selections'] = true;
                        $model_parent->addData($add_data);
                        $model_parent->save();
    ...
    protected function _getCustomersMainQuery() {
        ...
        $query = "SELECT * FROM _DBPRF_customer_entity WHERE entity_id >= 10562 AND entity_id <= 11681 AND entity_id > {$id_src} ORDER BY entity_id ASC LIMIT {$limit}";
        return $query;
    }
    ...
    protected function _getCustomersExtQuery($customers) {
        ...
        $ext_query = array(
            'core_store' => "SELECT * FROM _DBPRF_store WHERE code != 'admin' AND website_id IN {$web_ids}",
    ...
    public function convertCustomer($customer, $customersExt) {
        ...
        $cus_data['firstname'] = $customer['firstname'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['firstname'], 'value');
        $cus_data['middlename'] = $customer['middlename'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['middlename'], 'value');
        $cus_data['lastname'] = $customer['lastname'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['lastname'], 'value');
        ...
        $cus_data['password_hash'] = $customer['password_hash'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['password_hash'], 'value');
    ...
    public function afterSaveCustomer($customer_mage_id, $data, $customer, $customersExt) {
        ...
        if ($cusAdd) {
            ...
            $int = $this->getListFromListByField($customersExt['object']['customer_entity_int'], 'entity_id', $customer['entity_id']);
            $def_billing = $customer['default_billing'];// $this->getRowValueFromListByField($int, 'attribute_id', $attribute_cus['default_billing'], 'value');
            $def_shipping = $customer['default_shipping'];// $this->getRowValueFromListByField($int, 'attribute_id', $attribute_cus['default_shipping'], 'value');
            foreach ($cusAdd as $row) {
                ...
                $address['prefix'] = $row['prefix'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['prefix'], 'value');
                $address['firstname'] = $row['firstname'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['firstname'], 'value');
                $address['middlename'] = $row['middlename'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['middlename'], 'value');
                $address['lastname'] = $row['lastname'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['lastname'], 'value');
                $address['suffix'] = $row['suffix'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['suffix'], 'value');
                $address['country_id'] = $row['country_id'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['country_id'], 'value');
                $address['street'] = $row['street'];// $this->getRowValueFromListByField($text, 'attribute_id', $attribute['street'], 'value');
                $address['postcode'] = $row['postcode'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['postcode'], 'value');
                $address['city'] = $row['city'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['city'], 'value');
                $address['region'] = $row['region'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['region'], 'value');
                $address['region_id'] = $row['region_id'];// $this->getRegionId($address['region'], $address['country_id']);
                $address['telephone'] = $row['telephone'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['telephone'], 'value');
                $address['company'] = $row['company'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['company'], 'value');
                $address['fax'] = $row['fax'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['fax'], 'value');
                /**if (isset($attribute['vat_id'])) {
                    $address['vat_id'] = $row['vat_id'];// $this->getRowValueFromListByField($varchar, 'attribute_id', $attribute['vat_id'], 'value');
                }*/
                $address['vat_id'] = $row['vat_id'];
    ...
    protected function _getOrdersMainQuery() {
        ...
        $query = "SELECT * FROM _DBPRF_sales_order WHERE entity_id >= 16314 AND entity_id <= 19311 AND entity_id > {$id_src} ORDER BY entity_id ASC LIMIT {$limit}";
        return $query;
    }
    ...
    protected function _getOrdersExtQuery($orders) {
        ...
        $ext_query = array(
            'sales_flat_order_item' => "SELECT * FROM _DBPRF_sales_order_item WHERE order_id IN {$order_id_query} ORDER BY item_id ASC",
            'sales_flat_order_address' => "SELECT * FROM _DBPRF_sales_order_address WHERE parent_id IN {$order_id_query}",
            'sales_flat_order_status_history' => "SELECT * FROM _DBPRF_sales_order_status_history WHERE parent_id IN {$order_id_query} ORDER BY entity_id ASC",
            'sales_flat_order_payment' => "SELECT * FROM _DBPRF_sales_order_payment WHERE parent_id IN {$order_id_query}",
            'sales_flat_shipment_track' => "SELECT * FROM _DBPRF_sales_shipment_track WHERE order_id IN {$order_id_query}",
            'sales_flat_invoice' => "SELECT * FROM _DBPRF_sales_invoice WHERE order_id IN {$order_id_query}",
            'sales_shipment' => "SELECT * FROM _DBPRF_sales_shipment WHERE order_id IN {$order_id_query}"
        );
        return $ext_query;
    }
    ...
    protected function _getOrdersExtRelQuery($orders, $ordersExt) {
        ...
        $shipmentIds = $this->duplicateFieldValueFromList($ordersExt['object']['sales_shipment'], 'entity_id');
        $shipment_ids_query = $this->arrayToInCondition($shipmentIds);
        $ext_rel_query = array(
            'sales_flat_invoice_comment' => "SELECT * FROM _DBPRF_sales_invoice_comment WHERE parent_id IN {$invoice_ids_query}",
            'sales_flat_invoice_grid' => "SELECT * FROM _DBPRF_sales_invoice_grid WHERE entity_id IN {$invoice_ids_query}",
            'sales_flat_invoice_item' => "SELECT * FROM _DBPRF_sales_invoice_item WHERE parent_id IN {$invoice_ids_query} ORDER BY order_item_id ASC",
            'sales_shipment_comment' => "SELECT * FROM _DBPRF_sales_shipment_comment WHERE parent_id IN {$shipment_ids_query}",
            'sales_shipment_grid' => "SELECT * FROM _DBPRF_sales_shipment_grid WHERE entity_id IN {$shipment_ids_query}",
            'sales_shipment_item' => "SELECT * FROM _DBPRF_sales_shipment_item WHERE parent_id IN {$shipment_ids_query}",
            'sales_shipment_track' => "SELECT * FROM _DBPRF_sales_shipment_track WHERE parent_id IN {$shipment_ids_query}",
        );
        return $ext_rel_query;
    }
    ...
    public function afterSaveOrder($order_mage_id, $data, $order, $ordersExt) {
        ...
        $count = count($orderHistory);
        $orderHistory[$count]['status'] = $order['status'];
        $orderHistory[$count]['comment'] = '';
        $orderHistory[$count]['is_customer_notified'] = 1;
        $orderHistory[$count]['created_at'] = $order['created_at'];
        $orderHistory[$count]['updated_at'] = $order['updated_at'];
        foreach ($orderHistory as $key => $row) {
            ...
            $order_status_data['comment'] = (isset($row['comment'])) ? $row['comment'] : '';
            if ($key == 0) {
        ...
        $orderShipment = $this->getRowFromListByField($ordersExt['object']['sales_shipment'], 'order_id', $order['entity_id']);
        if($orderShipment){
            $store_id = ($orderShipment['store_id']) ? $this->_notice['config']['languages'][$orderShipment['store_id']] : $this->_notice['config']['languages'][$this->_notice['config']['default_lang']];
            unset($orderShipment['entity_id']);
            $orderShipment['store_id'] = $store_id;
            $orderShipment['order_id'] = $order_mage_id;
            $orderShipment['customer_id'] = $this->getMageIdCustomer($orderShipment['customer_id']);
            $this->insertTable('sales_shipment', $orderShipment);
            $shipment_id = $this->selectTableMax('sales_shipment', 'entity_id');
            
            $sales_shipment_track_data = $this->getRowFromListByField($ordersExt['object']['sales_shipment_track'], 'order_id', $order['entity_id']);
            unset($sales_shipment_track_data['entity_id']);
            $sales_shipment_track_data['parent_id'] = $shipment_id;
            $sales_shipment_track_data['order_id'] = $order_mage_id;
            $this->insertTable('sales_shipment_track', $sales_shipment_track_data);
            
            $sales_shipment_grid_data = $this->getRowFromListByField($ordersExt['object']['sales_shipment_grid'], 'entity_id', $orderShipment['entity_id']);
            $sales_shipment_grid_data['entity_id'] = $shipment_id;
            $sales_shipment_grid_data['order_id'] = $order_mage_id;
            $sales_shipment_grid_data['store_id'] = $store_id;
            $this->insertTable('sales_shipment_grid', $sales_shipment_grid_data);
            
            $sales_shipment_item = $this->getListFromListByField($ordersExt['object']['sales_shipment_item'], 'parent_id', $orderShipment['entity_id']);
            if($sales_shipment_item){
                foreach ($sales_shipment_item as $item){
                    $sales_shipment_item_data = $item;
                    $sales_shipment_item_data['parent_id'] = $shipment_id;
                    $sales_shipment_item_data['product_id'] = $this->getMageIdProduct($item['product_id']);
                    $this->insertTable('sales_shipment_item', $sales_shipment_item_data);
                }
            }
        }*/
