...
define([
    ...
    GiftOptionsPopup.prototype = {
        ...
        initialize: function () {
            $$('.action-link').each(
                function (el) {vent.observe(el, 'click', this.showItemGiftOptions.bind(this));},
                this
            );

            // Move gift options popup to start of body,
            // because soon it will contain FORM tag that can break DOM layout if within other FORM
            ...
            jQuery(this.giftOptionsWindow).dialog({
                ...
                open: function () {
                    ...
                    var topMargin = jQuery(this).closest('.ui-dialog').children('.ui-dialog-titlebar').outerHeight();
        ...
        setTitle: function (itemId) {
            var productTitleElement = $('quote_item_' + itemId + '_title');
            var productTitle = '';
    ...
    GiftMessageSet.prototype = {
        ...
        saveData: function (event) {
            ...
            if ($(this.sourcePrefix + this.id + '_form')) {
                ...
                var self = this;
                jQuery.when(quote.loadArea(['items'], true, data.toObject())).done(function () {
